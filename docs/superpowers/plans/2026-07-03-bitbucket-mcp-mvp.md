...
- [ ] **Step 3: `tests/conftest.py` に環境変数クリアの autouse フィクスチャを作成**
...
- [ ] **Step 1: `tests/conftest.py` にフィクスチャを追加**
...
- [ ] **Step 2: `tests/toolsets/__init__.py`（空）を作成し、失敗するテストを書く**
...
- [ ] **Step 1: `resolve_workspace` の失敗テストを書く**
...
- [ ] **Step 3: `repos.py` の register 末尾（read の `mcp.add_tool(...)` 群の直後、コメント行の位置）に write ブロックを実装**
...
- [ ] **Step 3: `pull_requests.py` の read ツールを実装**
...
- [ ] **Step 1: `models/__init__.py` を作成(InlineComment)**
...
- [ ] **Step 1: `models/__init__.py` に PipelineTarget を追加**
...
- [ ] **Step 3: `toolsets/__init__.py` をレジストリに置換**
...
- [ ] **全テスト**: `uv run pytest -v` → 全件 PASS。
- [ ] **型検査**: `uv run basedpyright` → 0 エラー。
- [ ] **Lint/整形**: `uv run ruff check .` → 0 エラー。
- [ ] **パッケージビルド**: `uv build` → wheel/sdist 生成成功。
