...
async def test_get_file_or_directory_preserves_braces_in_path(
    register_toolset, call_tool, httpx_mock: HTTPXMock
) -> None:
    httpx_mock.add_response(text="content")
    mcp, _ = register_toolset(repos.register, default_workspace="ws1")

    _, structured = await call_tool(
        mcp,
        "get_file_or_directory",
        {"repo_slug": "r", "commit": "abc", "path": "dir/{literal}.txt"},
    )

    request = httpx_mock.get_request()
    assert request is not None
    assert request.url.path == "/2.0/repositories/ws1/r/src/abc/dir/{literal}.txt"
